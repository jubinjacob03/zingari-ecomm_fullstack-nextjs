...
export default async function handler(req, res) {
  ...
  for (const productId of uniqueProductIds) {
    ...
    for (const selectedSize of selectedSizes) {
      const quantity = cartProducts.filter(p =>
        p.productId === productId && p.selectedSize === selectedSize).length || 0;
      ...
      if (quantity > 0 && productInfo) {
        line_items.push({
          ...
          price_data: {
            currency: 'inr',
            product_data: { name: productInfo.title, metadata: { selectedSize } },
